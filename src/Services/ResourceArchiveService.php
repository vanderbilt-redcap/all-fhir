...
class ResourceArchiveService
{
    ...
    /**
     * Download a completed archive file
     * 
     * Handles both background-processed archives (via queue tasks) and immediate archives
     * (created synchronously). For immediate archives, the archiveId is actually the 
     * unique archive identifier from the ArchivePackager.
     * 
     * @param string $archiveId The archive task identifier or direct archive ID
     * @return string|null File path for download or null if not available
     */
    public function downloadArchive(string $archiveId): ?string
    {
        ...
        // If no queue task found or file not found, try direct file path approach
        // This handles immediate archives that weren't processed via queue
        $directFilePath = $this->resolveDirectArchiveFilePath($archiveId);
        
        if ($directFilePath && file_exists($directFilePath) && is_readable($directFilePath)) {
    ...
    /**
     * Resolve direct file path for immediate archives
     * 
     * First checks stored immediate archive info, then falls back to pattern matching
     * in the temp directory if the archive wasn't stored (legacy support).
     * 
     * @param string $archiveId Archive identifier from immediate creation
     * @return string|null File path if it can be resolved
     */
    private function resolveDirectArchiveFilePath(string $archiveId): ?string
    {
        // First, check stored immediate archive info
        $immediateArchives = $this->module->getProjectSetting('immediate_archives') ?? [];
        
        if (isset($immediateArchives[$archiveId])) {
            $storedInfo = $immediateArchives[$archiveId];
            $filePath = $storedInfo['file_path'];
            
            // Verify the stored file path still exists
            if (file_exists($filePath) && is_readable($filePath)) {
                return $filePath;
            }
        }
        
        // Fallback: Try different possible file path patterns for legacy archives
        $tempDir = sys_get_temp_dir();
        
        // Pattern 1: Direct archive ID as filename (from ArchivePackager)
        $possiblePaths = [
            $tempDir . DIRECTORY_SEPARATOR . $archiveId . '.zip',
            $tempDir . DIRECTORY_SEPARATOR . $archiveId,
        ];
        
        // Pattern 2: If archive ID looks like it contains a filename already
        if (strpos($archiveId, '.') !== false || strpos($archiveId, DIRECTORY_SEPARATOR) !== false) {
            $possiblePaths[] = $archiveId; // Use as direct path
            
            // If it's a relative path, try making it absolute
            if (!str_starts_with($archiveId, '/')) {
                $possiblePaths[] = $tempDir . DIRECTORY_SEPARATOR . $archiveId;
            }
        }
    ...
    /**
     * Store immediate archive information for later retrieval
     * 
